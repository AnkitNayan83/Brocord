...
const ServerChannel = ({ channel, server, role }: ServerChannelProps) => {
    ...
    return (
        ...
        >
            <Icon className="flex-shrink-0 w-5 h-5 text-zinc-500 dakr:text-zinc-400" />
            <p
                className={cn(
            ...
            {channel.name !== "general" && role !== MemberRole.GUEST && (
                <div className="ml-auto flex items-center gap-x-2">
                    ...
                    <ActionTooltip label="delete">
                        <Trash className="hidden group-hover:block w-4 h-4 text-zinc-500 hover:text-zinc-600 dark:text-zinc-400 dark:hover:text-zinc-300 transistion" />
                    </ActionTooltip>
                </div>
            )}
